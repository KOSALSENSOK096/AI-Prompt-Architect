...
const Navbar: React.FC = () => {
  ...
  return (
    <>
      <nav className="bg-white/80 dark:bg-slate-800/80 backdrop-blur-lg shadow-lg dark:shadow-slate-700/30 sticky top-0 z-50 transition-all duration-300">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex items-center justify-between h-20">
            <div className="flex items-center">
              <Link to="/" className="flex-shrink-0 flex items-center group">
                <LogoIcon className="h-10 w-auto text-primary dark:text-primary-light group-hover:text-primary-dark dark:group-hover:text-primary transition-all transform group-hover:scale-105" />
                <span className="ml-3 text-xl font-bold text-slate-800 dark:text-slate-100 group-hover:text-primary-dark dark:group-hover:text-primary transition-all">{APP_NAME}</span>
              </Link>
            </div>
              ...
              <div className="ml-10 flex items-baseline space-x-2 lg:space-x-3">
                {filteredNavLinks.map((link) => (
                  <Link
                    key={link.label}
                    to={link.path}
                    className={`px-4 py-2.5 rounded-xl text-sm font-medium transition-all flex items-center hover:scale-105
                      ${location.pathname === link.path 
                        ? 'bg-primary/10 text-primary-dark dark:bg-primary-dark/20 dark:text-primary-light' 
                  ...
                  >
                    {link.icon && <link.icon className="w-4 h-4 mr-2 opacity-80" />}
                    {link.label}
                  </Link>
                ))}
              </div>
            ...
            <div className="hidden md:flex items-center space-x-4">
               <button
                onClick={toggleTheme}
                className="p-2.5 rounded-xl text-slate-500 dark:text-slate-400 hover:bg-slate-100 dark:hover:bg-slate-700/50 focus:outline-none transition-all hover:scale-105"
                aria-label={theme === 'light' ? 'Switch to dark mode' : 'Switch to light mode'}
              >
                {theme === 'light' ? <MoonIcon className="h-5 w-5" /> : <SunIcon className="h-5 w-5" />}
              </button>
              {authIsLoading ? (
                 <div className="animate-pulse bg-slate-200 dark:bg-slate-700 h-10 w-24 rounded-xl"></div>
              ) : currentUser ? (
                <div className="flex items-center space-x-4">
                  <span className="text-sm font-medium text-slate-600 dark:text-slate-400">Hi, {currentUser.username}!</span>
                  <button
                    onClick={handleLogout}
                    className="px-4 py-2.5 rounded-xl text-sm font-medium text-white bg-secondary hover:bg-secondary-dark dark:bg-secondary-dark dark:hover:bg-secondary transition-all hover:scale-105"
                  >
                    Logout
                  </button>
                </div>
                ...
                <div className="space-x-3">
                  <button
                    ...
                    className="px-4 py-2.5 rounded-xl text-sm font-medium text-primary dark:text-primary-light hover:bg-primary/10 dark:hover:bg-primary-dark/20 transition-all hover:scale-105 border-2 border-primary/20 dark:border-primary-light/20"
                  >
                    Login
                  </button>
                  <button
                    onClick={() => openAuthModal('register')}
                    className="px-4 py-2.5 rounded-xl text-sm font-medium text-white bg-primary hover:bg-primary-dark dark:hover:bg-primary-light dark:text-slate-900 transition-all hover:scale-105"
                  >
                    Register
                  </button>
                </div>
